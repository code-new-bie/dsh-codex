...
mkdir -p "$OUT_DIR"
# Ship without debug info: ~4x smaller artifacts, identical behavior. The
# override stays user-controlled for anyone who needs a debuggable build.
...
if command -v strip >/dev/null 2>&1; then
  strip "$OUT_DIR/dshx-tui" "$OUT_DIR/dshx-ipc-bridge"
fi
